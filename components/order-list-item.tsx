...
import { Calendar, CheckCircle, Package, Truck, XCircle, ChevronDown, ChevronUp } from "lucide-react"
...
const statusConfig = {
  ...
  pickup_requested: { label: 'Pickup Requested', color: 'bg-purple-500', icon: Truck },
  delivered: { label: 'Delivered', color: 'bg-gray-500', icon: CheckCircle },
  rejected: { label: 'Rejected', color: 'bg-red-500', icon: XCircle },
}

export default function OrderListItem({ order, onUpdate, isAdmin, onAccept, onReject }: OrderListItemProps) {
      ...
      {isExpanded && (
        <CardContent className="space-y-4 pt-0 pb-4 border-t">
          <div className="grid gap-3">
            <div>
              <p className="text-sm font-medium text-muted-foreground">Order Message</p>
          ...
          {isAdmin && (
            <div className="flex flex-col sm:flex-row gap-2 pt-2">
              ...
              {(order.status === 'prepared' || order.status === 'pickup_requested') && (
